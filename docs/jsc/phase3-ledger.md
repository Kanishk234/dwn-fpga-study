...
| ~~Do the non-DWN rows include an input encoder?~~ | ✅ **Closed 2026-08-11 — duplicate of the row below**, which resolved it: `n/a` is correct and sourced. Left here struck through rather than deleted, because it stayed open for a day after the question had already been answered. |
...
| ~~Is the hls4ml 76.2% / 63,251 number real?~~ | ✅ **Closed 2026-08-10.** Traced to Fahim et al. via PolyLUT (arXiv:2309.02334) Table IV: **76%, 63,251 LUTs, 4,394 FF, 38 DSP, 0 BRAM, 200 MHz, 45 ns**. Two corrections fell out of it — the accuracy is reported as 76%, not 76.2%, and it uses 38 DSPs, which our brief never recorded. Marked `verified` in the JSON. |
