...
| ~~**Baud actually achieved**~~ | ✅ **5 Mbaud, and the sweep is deliberately skipped.** Not a measured ceiling — a working rate | **Decided 2026-08-12:** the I/O wall is 243,271×, so the link is already the entire cost and a faster link changes no reported number. JSC swept baud because 480 s → 11.2 s made the test set practical to run at all; MNIST's full pass is 31.9 s at 5 Mbaud, which is not a constraint on anything. Revisit only if a Phase 2 config makes a full pass painful |
