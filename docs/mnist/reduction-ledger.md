...
`training/mnist_noise_floor_kaggle.ipynb`, generated by `scripts/mk_noise_floor_nb.py`. **16 runs
— four configurations × four seeds**, ~2 h. Nothing new is trained: every configuration already
exists at seed 20260811, and every `tau` follows the power law R4 confirmed, so nothing here is
`tau`-confounded.

`train_one` seeds before it builds, so exactly three things move — LUT init, mapping init (hence
which bits each node reads), and batch order. The thermometer fit is a quantile computation and
the split is a fixed slice; neither depends on the seed.

| architecture | group | why this one |
|---|---|---|
| `1x300` τ=1.678, 96.77% | 30 | the model in the bitstream, and the one whose accuracy just moved +0.63 pp |
| `1x1000` τ=3.333, 97.97% | 100 | the `tau` anchor, mid-ladder |
| `1x2000` τ=4.948, 98.26% | 200 | top of the ladder |
| `2x[2000,1000]` τ=3.333, 98.32% | 100 | best in the study, and one end of the +0.06 pp claim |

Four widths rather than one because **variance need not be constant across model size**: JSC took
its 0.15 pp from a single configuration and applied it everywhere, and a floor measured on 300
nodes does not license a comparison between two 3,000-node models.

**It retrains seed 20260811 in-session rather than reusing the recorded number.** That is the
better call and worth stating: it makes run #1 of each row a **reproduction check** against a
figure produced weeks and several refactors ago. If `1x300` does not come back at 96.77%, the
disagreement is itself the measurement — CUDA kernel non-determinism is part of the floor this is
trying to bound, and reusing the old checkpoint would have hidden it.

⚠️ **One gap, recorded rather than fixed.** Neither endpoint of the **−0.27 pp** claim below
(`1x500` vs `3x[2000,300,500]`) is in the grid, so that gap is bounded by a floor measured
elsewhere rather than at its own operating point. Given the grid spans 300 to 3,000 nodes that is a
reasonable extrapolation, but it is an extrapolation. Both endpoints are group-50 configurations,
and every row here is group 30 or wider.

#### What this ledger currently asserts without evidence

| claim in this file | gap | survives a floor of… |
|---|---|---|
| `2x[2000,1000]` is the best model in the study | **+0.06 pp** | <0.06 — **very unlikely** |
| `1x2000`'s `tau` correction helped | **+0.09 pp** | <0.09 |
| **`1x500` beats the best taper** | **−0.27 pp** | <0.27 — **the one to watch** |
| the board model gained from the `tau` fix | **+0.63 pp** | <0.63 — should hold |

⚠️ **The domination conclusion is the exposed one**, since it turns on 0.27 pp. Worth noting in
advance that it survives losing that number: if the floor lands above 0.27, the honest statement
becomes *"a taper is not better than a narrower single layer and costs 5× the nodes"* — the same
engineering conclusion, reached on area alone, with no accuracy claim in it.
...
- `training/mnist_noise_floor_kaggle.ipynb` — the noise-floor grid (R6), generated by
  `scripts/mk_noise_floor_nb.py`. Diagnostic runs: **nothing it produces should be exported,
  synthesized, or land in `training/artifacts/`**
